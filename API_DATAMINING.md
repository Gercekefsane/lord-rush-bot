# Lords Rush — API Change Timeline
...
An **append-only** record of every change we observe in Lords Rush's gift-code API. Entries are only ever added — nothing is edited or deleted — so a removed endpoint or a retired payload shape keeps its row here permanently. Newest first.
...
|---|---|:--:|---|
| 24/07/2026 09:00 | `/gift_code` | ![added](https://img.shields.io/badge/added-2ea44f?style=flat-square) | Lords Rush gift-code API mapped — single signed `POST`, params `sign, fid, cdk, kid, time`, `sign = md5(sorted params + per-game secret key)`, unix-**seconds** timestamp, `kid` required, **no captcha**, **no login step** |
...
> **2026-07-24 — baseline mapping.** Lords Rush gift-code API mapped: `/api/gift_code` + `/api/gift_code_config` live; `/api/player` + `/api/captcha` return 404 (removed, mirroring WOS/KS 2026-07-21); single signed `POST`, `kid` required, seconds timestamp, no captcha.
