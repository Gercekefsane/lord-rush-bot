...
- **`/register [FID]`**: Members register with their Fighter ID (FID); because Lord Rush codes are redeemed per kingdom, registration collects your **kingdom number** up front so codes always reach the right account
...
- **Website Integration**: Live Lord Rush gift codes are published to [woscontrol.com/codes](https://woscontrol.com/codes), auto-updated every 5 minutes
...
### 🔄 Changed
- **Single Signed Redemption Flow**: Lord Rush redemption is a single signed call (`sign = md5(sorted params + a per-game secret key)`, unix-seconds timestamp) — no player-info lookup and no captcha step, matching Century Games' current backend for this game
- **Game-Type Awareness**: All operations are scoped by game type (`lr`) so Lord Rush alliances only ever see Lord Rush codes
