...
|----------|------|------|--------|
| 🇬🇧 English | `en` | [`locales/en.json`](locales/en.json) | ✅ Complete |
| 🇯🇵 日本語 | `ja` | [`locales/ja.json`](locales/ja.json) | ✅ Complete |
| 🇰🇷 한국어 | `ko` | [`locales/ko.json`](locales/ko.json) | ✅ Complete |
| 🇷🇺 Русский | `ru` | [`locales/ru.json`](locales/ru.json) | ✅ Complete |
| 🇹🇷 Türkçe | `tr` | [`locales/tr.json`](locales/tr.json) | ✅ Complete |
...
Copy `locales/en.json` as your starting template — English is always the most up-to-date:
...
Open `locales/XX.json` and translate **every value** (right side of `:`) while keeping **every key** (left side of `:`) unchanged.
...
```bash
python -c "import json; json.load(open('locales/XX.json', encoding='utf-8')); print('✅ Valid JSON')"
```
...
| Members | `member_*`, `addmember_*` | Member operations |
| Gift Codes | `giftcode_*`, `codes_*` | Gift code system messages |
| Leaderboard | `leaderboard_*` | Ranking messages |
...
- **Kingdom** — translate to your language's in-game term (this is the `kid`)
